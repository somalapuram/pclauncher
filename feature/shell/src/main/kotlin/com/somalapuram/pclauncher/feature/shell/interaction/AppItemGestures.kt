...
fun Modifier.appItemGestures(
    ...
    awaitEachGesture {
        ...
        if (down.type == PointerType.Mouse) {
            val dragged = trackMouseDrag(down.id, slop, down.position, onDragStart, onDrag)
        ...
        // null from withTimeoutOrNull means the timer won; Unspecified means the finger lifted;
        // anything else is the travel that crossed slop.
        val movedBeforeTimeout = withTimeoutOrNull(longPressMillis) {
            awaitSlopOrUp(down.id, slop, claimOnSlop = false) ?: Offset.Unspecified
        }

        when {
            // Scrolling. Hands off entirely.
            movedBeforeTimeout != null && movedBeforeTimeout != Offset.Unspecified ->
                return@awaitEachGesture

            // Lifted before the timer without moving: an ordinary tap.
            movedBeforeTimeout == Offset.Unspecified -> onClick()
            ...
            // move is a drag, hold and release is the menu.
                ...
                val travelled = awaitSlopOrUp(down.id, slop, claimOnSlop = true)
                if (travelled != null) {
...
/** Mouse path: start a drag the moment movement passes slop. Returns true if one happened. */
